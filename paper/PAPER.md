...
### 5.9 Statistical certificates

Four results above admit exact statistical treatment (full derivations and Wilson 95% intervals for all headline cells: `analysis/statistical_certificates.md`). First, under the null that SmolLM's low-confidence flags are independent of correctness, the probability that all 17 flagged cases fall on errors is **≈ 1.1 × 10⁻¹⁰** by exact hypergeometric calculation (C(58,17)/C(200,17)) — strong evidence for a confidence-channel dissociation: confidence carries correctness signal while action selection fails the ABSTAIN/ESCALATE distinction. Second, Qwen's analogous pattern (11/11 mid-confidence statements correct) has exact probability ≈ 3.0 × 10⁻³ — supporting, not headline, evidence. Third, a construction-level bound: each v2 task type contains 30 standard tasks with one gold action and 20 control tasks with a different gold action, so any policy mapping task type alone to a fixed action satisfies **standard accuracy + control accuracy ≤ 1**; the overall-optimal such policy scores 0.600 overall with 0.000 on controls. The observed sums — Qwen 1.13, SmolLM 1.40, Granite 1.40 — exceed this frontier, establishing deductively that all three models use information beyond task type. Fourth, key cells carry tight intervals: SmolLM's gold-ABSTAIN accuracy of 0/50 is bounded below 0.071 (Wilson 95%), and Granite's escalation contrast (0.167 standard vs 0.600 control, n=60/20) has non-overlapping intervals. One caveat stated plainly: the 0.8 confidence threshold was selected after observing v1; the v2 result is therefore an out-of-sample replication of the v1 pattern (4/4 → 17/17) rather than a pre-registered threshold test. These are claims about behavior in this run under stated nulls — not claims of metacognition in any rich sense, self-awareness, consciousness, AGI, deployment safety, or real-world generalization.
